...
for(const icon of manifest.icons){
 const source=`assets/icons/light/${icon.name}.svg`,dest=`assets/motion/icons/${icon.name}.svg`;
 const original=await fs.readFile(path.join(root,source),'utf8');
 const withoutSurface=original.replace(/<rect width="60" height="60" rx="10" fill="white"\/>/,'');
 ...
 entries.push({name:icon.name,file:dest,source,box:46,viewBox:view,policy:'Original paths; outer surface removed; square crop around measured alpha bounds plus 1.2 source-pixel margin.'});
}
