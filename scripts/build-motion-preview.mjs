...
for(const lang of ['en','zh']){
 const qa=path.join(root,`output/motion-${lang}/qa`),report=JSON.parse(await fs.readFile(path.join(qa,'report.json'),'utf8'));
 ...
 const output=path.join(root,`assets/previews/motion-slides.${lang}.webp`);
