...
Dark does not mean gray only. `meta.palette` / `slide.palette` select `neutral`, `teal`, `cobalt` or `lime`; non-neutral palettes default to a solid brand-color backdrop. Theme controls contrast independently: deep brand colors use Dark/light text, bright lime uses Light/dark text. Every registered layout supports every palette, but defaults are intentionally biased: bright lime mainly for a large centered metric/keyword, Lists mainly for a solid brand color, openings/closings mainly Light or solid brand color, and ordinary information scenes mainly calm neutral or solid brand colors. Do not rotate palettes for variety. Explicit lower-priority combinations remain valid when the content justifies them. The only distinct optional brand gradient is the deep-blue `#101B27 → #14236F`; other `brand-gradient` inputs retain compatibility but resolve to their solid palette. See `references/motion/scene-planning.md` and the brand-color section of `references/motion/themes-and-combinations.md`.
