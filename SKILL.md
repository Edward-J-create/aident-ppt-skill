...
Dark does not mean gray only. `meta.palette` / `slide.palette` select `neutral`, `teal`, `cobalt` or `lime`; non-neutral palettes default to a brand-color solid backdrop, or use `background: "brand-gradient"`. Theme controls contrast independently: deep brand colors use Dark/light text, bright lime uses Light/dark text. Use matching panel paints and theme-safe assets. Palettes are optional editorial choices, not required brand claims or automatic alternating shots. See the brand-color section of `references/motion/themes-and-combinations.md`.
